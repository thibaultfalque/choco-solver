...
/**
 * This class is inspired from : <cite> Inspired from "Unifying search
 * algorithms for CSP" N. Jussien and O. Lhomme, Technical report 02-3-INFO, EMN
 * </cite>
 * <p>
 * It declares a search loop made of three components:
 * <ul>
 * <li>Propagate: it aims at propagating information throughout the constraint
 * network when a decision is made,</li>
 * <li>Learn: it aims at ensuring that the search mechanism will avoid (as much
 * as possible) to get back to states that have been explored and proved to be
 * solution-less,</li>
 * <li>Move: aims at, unlike other ones, not pruning the search space but rather
 * exploring it.</li>
 * <p>
 * </ul>
 * <p>
...
public class Solver implements ISolver, IMeasures, IOutputFactory {
	...
	/**
	 * Index of the initial world, before initialization. May be different from 0 if
	 * some external backups have been made.
	 */
	...
	/**
	 * List of stopping criteria. When at least one is satisfied, the search loop
	 * ends.
	 */
	protected List<Criterion> criteria;

	/**
	 * Indicates if the default search loop is in use (set to <tt>true</tt> in that
	 * case).
	 */
	private boolean defaultSearch = false;

	/**
	 * Indicates if a complementary search strategy should be added (set to
	 * <tt>true</tt> in that case).
	 */
	...
	/**
	 * Problem feasbility: - UNDEFINED if unknown, - TRUE if satisfiable, - FALSE if
	 * unsatisfiable
	 */
	...
	/**
	 * Set to <tt>true</tt> when no more reparation can be achieved, ie entire
	 ...
	 * <p>
	 * Default configuration: - SATISFACTION : Computes a feasible solution. Use
	 * while(solve()) to enumerate all solutions. - OPTIMISATION : Computes a
	 * feasible solution, wrt to the objective defined. Use while(solve()) to find
	 * the optimal solution. Indeed, each new solution improves the objective. If no
	 * new solution is found (and no stop criterion encountered), the last one is
	...
	/**
	 * Preparation of the search: - start time recording, - store root world - push
	 * a back up world, - run the initial propagation, - initialize the Move and the
	 * search strategy
	 */
	protected boolean initialize() {
		...
		if (mModel.getSettings().checkDeclaredConstraints()) {
			...
			if (instances != null) {
				Optional<Constraint> undeclared = instances.stream().filter(c -> (c.getStatus() == FREE)).findFirst();
				if (undeclared.isPresent()) {
					logger.white().println("At least one constraint is free, i.e., neither posted or reified. ).");
					instances.stream().filter(c -> c.getStatus() == FREE)
	...
	/**
	 * This method is called after the initial propagation and before the search
	 * loop starts. It sequentially applies Arc Consistency on every combination of
	 * (variable, value). If a value is not supported by any other variable, it is
	 * removed from the domain of the variable. The method ends when the time limit
	 * is reached or when all combination have been checked.
	 * 
	...
	public void preprocessing(long timeLimitInMS) {
		if (!getEngine().isInitialized()) {
			throw new SolverException(
					"A call to solver.propagate() must be done before calling solver.preprocessing()");
		}
		...
		loop: for (int i = 0; i < ivars.length; i++) {
			IntVar v = ivars[i];
	 ...
	 * {@link org.chocosolver.solver.search.loop.move.Move#repair(Solver)}
	 * call)</li>
	 * <li>then, a first propagation ensures that, if learning is on, the unit
	 * no-good learnt on failure filters,
	 * <li>the cut is posted before applying the decision to ensure good nogood, and
	 * a second propagation ensures the cut is taken into account</li>
	...
	/**
	 * Search loop propagation phase. This needs to be distinguished from
	 * {@link #propagate()}
	 *
	...
	private boolean validate() {
		if (!getModel().getSettings().checkModel(this)) {
			throw new InvalidSolutionException("The current solution does not satisfy the checker."
					...
					+ Reporting.fullReport(mModel), mModel);
		}
		feasible = TRUE;
	...
	/**
	 * Close the search: - set satisfaction - update statistics
	 */
	...
	/**
	 * <p>
	 * Resetting a solver to the state just before running the last resolution
	 * instruction. That is, {@link Learn}, {@link Move} and {@link Search} are kept
	 * as declared. {@link ISearchMonitor} are also kept plugged to the search loop.
	 * </p>
	 * <p>
	 ...
	 * <li>synchronize {@link #dpath} to erase out-dated decisions, presumably all
	 * of them</li>
	 * <li>reset bounds of {@link #objectivemanager} (calling
	 * {@link IObjectiveManager#resetBestBounds()}</li>
	...
	/**
	 * Propagates constraints and related events through the constraint network
	 * until a fix point is find, or a contradiction is detected.
	 *
	 * @throws ContradictionException inconsistency is detected, the problem has no
	 *                                solution with the current set of domains and
	 *                                constraints.
	 * @implNote The propagation engine is ensured to be empty (no pending events)
	 *           after this method. Indeed, if no contradiction occurs, a fix point
	 *           is reached. Otherwise, a call to {@link PropagationEngine#flush()}
	 *           is made.
	 */
	...
	/**
	 * Return the minimum conflicting set from a conflicting set that is causing
	 * contradiction.
	 *
	 * @param conflictingSet the super-set of constraints causing contradiction
	 * @return minimumConflictingSet of constraints (the root cause of
	 *         contradiction)
	...
	/**
	 * Sets the following action in the search to be a restart instruction. Note
	 * that the restart may not be immediate
	 */
	public void restart() {
	...
	/**
	 * Retrieves the state of the root node (after the initial propagation) Has an
	 * immediate effect
	 */
	...
	/**
	 * <p>
	 * Move forward in the search space by adding a new decision. A call to this
	 * method will :
	 * <ol>
	 ...
	 * <p>
	 * In case of success, a call moveForward(Decision) is possible. Otherwise, a
	 * call {@link #moveBackward()} is required to keep on exploring the search
	 * space. If no such call is done, the state maybe inconsistent with the
	 * decision path.
	 * </p>
	 * <p>
	 ...
	 * <li>propagate</li>
	 * </ol>
	 * If step 2. is not possible or step 3. throws a failure, the last decision of
	 * the decision path is popped and the three-step loop is applied until a
	 * successful refutation or emptying decision path.
	 * <p>
	...
	/**
	 * Solving is executing if the search state is different from NEW, that is, if
	 * it has started to branch decisions. A double check for execution is done
	 * looking if the environment trailing has started as well.
	 *
	...
	/**
	 * @return <tt>true</tt> if the search loops ends unexpectedly (externally
	 *         killed, for instance).
	 */
	...
	/**
	 * @return <tt>true</tt> if the search loops encountered at least one of the
	 *         stop criteria declared.
	 */
	...
	/**
	 * Returns a reference to the measures recorder. This enables to get, for
	 * instance, the number of solutions found, time count, etc.
	 *
	...
	/**
	 * Returns information on the feasibility of the current problem defined by the
	 * solver.
	 * <p>
	 * Possible back values are: <br/>
	 * - {@link ESat#TRUE}: a solution has been found, <br/>
	 * - {@link ESat#FALSE}: the CSP has been proven to have no solution, <br/>
	 * - {@link ESat#UNDEFINED}: no solution has been found so far (within given
	 * limits) without proving the unfeasibility, though.
	 *
	 * @return an {@link ESat}.
	 */
	 ...
	 * <p>
	 * Given the current domains, it can return a value among: <br/>
	 * - {@link ESat#TRUE}: all constraints of the CSP are satisfied for sure, <br/>
	 * - {@link ESat#FALSE}: at least one constraint of the CSP is not satisfied.
	 * <br/>
	 * - {@link ESat#UNDEFINED}: neither satisfiability nor unsatisfiability could
	 * be proven so far.
	 * <p>
	 ...
	 *         <tt>ESat.UNDEFINED</tt> neither satisfiability nor unsatisfiability
	...
	/**
	 * @return the current declared restart policy or
	 *         {@link AbstractRestart#NO_RESTART}
	 */
	...
	/**
	 * Clear the declared restart strategy. Consequently, no restarting will occur.
	 *
	 * @implNote replace the declared restart policy by
	 *           {@link AbstractRestart#NO_RESTART}
	 */
	...
	/**
	 * Override the default search strategies to use in {@code this}. In case many
	 * strategies are given, they will be called in sequence: The first strategy in
	 * parameter is first called to compute a decision, if possible. If it cannot
	 * provide a new decision, the second strategy is called ... and so on, until
	 * the last strategy.
	 * <p>
	 *
	...
	public void setSearch(AbstractStrategy... strategies) {
		...
		if (M.getChildMoves().size() > 1) {
			...
			strategies = Arrays.stream(strategies).filter(Objects::nonNull)
					.flatMap(s -> (s instanceof StrategiesSequencer)
							? Arrays.stream(((StrategiesSequencer<?>) s).getStrategies())
							: Stream.of(s))
	...
	/**
	 * Attaches a propagation engine {@code this}. It overrides the previously
	 * defined one, only if no propagation was done yet. Indeed, some incremental
	 * propagators may have set up their internal structure, which cannot be set up
	 * twice safely.
	 * <p>
	 * If propagation was done calling {@link #solve()}, calling {@link #reset()}
	 * enables to set the propagation engine anew.
	 * <p>
	 * If propagation was done "manually" (calling {@link #propagate()}, then
	 * nothing can be done.
	 *
	...
	public void setEngine(PropagationEngine propagationEngine) {
		if (!engine.isInitialized() || getEnvironment().getWorldIndex() == rootWorldIndex) {
	...
	/**
	 * Declare a <i>warm start</i> strategy that consists of a set of variables and
	 * a set of values. It allows to define either a solution or at least a partial
	 * solution in order to drive the search toward a solution.
	 * <p>
	...
	/**
	 * Adds a stop criterion, which, when met, stops the search loop. There can be
	 * multiple stop criteria, a logical OR is then applied. The stop criteria are
	 * declared to the search loop just before launching the search, the previously
	 * defined ones are erased.
	 * <p>
	...
	/**
	 * Removes one or many stop criterion from the one to declare to the search
	 * loop.
	 *
	...
	/**
	 * Empties the list of stop criteria declared. This is automatically called on
	 * {@link #reset()}.
	 */
	@SuppressWarnings("WeakerAccess")
	 ...
	 * <p>
	 * There is no check if there are any duplicates. A search monitor added during
	 * while the resolution has started will not be taken into account.
	 *
	...
	/**
	 * The first call to this method will create a new solution based on all
	 * variables of the model and attach it to this. Next calls return the solution
	 * instance.
	 *
	...
	/**
	 * Return the current used logger. By default, logger prints to
	 * {@link System#out}. Any trace from choco-solver are redirected to this
	 * logger.
	 *
	...
	/**
	 * Defines whether (when {@code ansi} is set to {@code true}) or not ANSI tags
	 * are added to any trace from choco-solver.
	 *
